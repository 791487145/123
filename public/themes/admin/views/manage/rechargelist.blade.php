...
<div class="  well">
    <form class="form-inline search-group" role="form" method="get" action="{!! url('manage/rechargeList') !!}">
        ...
        <div class="form-group search-list">
            ...
            <select name="status">
                <option value="0" @if(isset($search['status']) && $search['status'] == '0')selected="selected"@endif>未付款</option>
                <option value="1" @if(isset($search['status']) && $search['status'] == '1')selected="selected"@endif>已付款</option>
            </select>
        </div>
...
<div class="table-responsive">
    <table id="sample-table-1" class="table table-striped table-bordered table-hover">
        ...
        <tr>
            ...
            <td>
                @if($item->status == 0) 待付款 @else 已付款 @endif
            </td>
            {{--<td>--}}
