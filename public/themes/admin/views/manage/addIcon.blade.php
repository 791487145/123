...
<div class="">
    <div class="g-backrealdetails clearfix bor-border">
        <form class="form-horizontal clearfix registerform" role="form" action="{{ url('manage/icon') }}" method="post" enctype="multipart/form-data">
            {!! csrf_field() !!}
                ...
                <p class="col-sm-4">
                    <select name="type" class="col-xs-10 col-sm-5">
                        @foreach($icon_type as $v)
                            <option value="{{ $v['type_name'] }}">{{ $v['describe'] }}</option>
                        @endforeach
                    </select>
                </p>
            </div>
