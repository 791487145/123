            ...
            <div class="form-inline search-group" >
                ...
                <div class="form-group search-list">
                    ...
                    <select name="status">
                        ...
                        <option value="2" @if(isset($merge['status']) && $merge['status'] == 2)selected="selected"@endif>认证成功</option>
                        <option value="3" @if(isset($merge['status']) && $merge['status'] == 3)selected="selected"@endif>认证失败</option>
                    </select>
                </div>
            </div>
        </form>
        ...
        <table id="sample-table" class="table table-striped table-bordered table-hover">
            ...
            @foreach($alipay as $item)
                <tr>
                    ...
                    <td>
                        @if($item->auth_time){!! $item->auth_time !!} @endif
                    </td>
                    <td>
                        @if($item->realname_status == 0)
                            <span class="label label-sm label-warning">待审核</span>
                        @elseif($item->realname_status == 1)
                            <span class="label label-sm label-success">认证成功</span>
                        @elseif($item->realname_status == 2)
                            <span class="label label-sm label-danger">认证失败</span>
                        @endif
                    </td>
                    <td>
                        <div class=" btn-group">
                            @if($item->status == 1 && $item->realname_status ==1)
                                <a title="通过" href="/manage/alipayAuthHandle/{!! $item->id !!}/pass" class="btn btn-xs btn-success">
