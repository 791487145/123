...
<form class="form-horizontal" role="form" action="/manage/companyUpdate/{{$company->id}}" enctype="multipart/form-data" method="post">
	{!! csrf_field() !!}
	...
	<div class="bankAuth-bottom clearfix col-xs-12">
		...
		<p class="col-sm-4">
			<input type="text" id="form-field-1" name="name"  class="col-xs-10 col-sm-5" value="{{$company->name}}">
	...
	<div class="bankAuth-bottom clearfix col-xs-12">
		...
		<div class="bankAuth-bottom clearfix col-xs-12">
			...
			<select name="scale">
				@foreach($company_scales as $company_scale)
					<option {{$company->scale == $company_scale->id ? 'selected': ''}} value="{{$company_scale->id}}">{{$company_scale->scale}}</option>
				@endforeach
			</select>
			</p>
		</div>
		<div class="bankAuth-bottom clearfix col-xs-12">
			...
			<select name="industry">
				@foreach($company_industies as $company_industy)
					<option {{$company->industry == $company_industy->id ? 'selected': ''}} value="{{$company_industy->id}}">{{$company_industy->industry}}</option>
				@endforeach
			</select>
			</p>
		</div>
		<div class="bankAuth-bottom clearfix col-xs-12">
			...
			<select name="financing">
				@foreach($company_financings as $company_financing)
					<option {{$company->financing == $company_financing->id ? 'selected': ''}} value="{{$company_financing->id}}">{{$company_financing->financing}}</option>
				@endforeach
			</select>
			</p>
		</div>
		...
		<div class="bankAuth-bottom clearfix col-xs-12">
			...
			<p class="col-sm-4">
				<input type="text" placeholder="" id="form-field-1" name="email"  class="col-xs-10 col-sm-5" value="{{$company->email}}">
				<span class="help-inline col-xs-12 col-sm-7"><i class="light-red ace-icon fa fa-asterisk"></i></span>
			</p>
		</div>
				...
				<div class="row">
					...
					<p class="col-sm-4">
						<select name="province" id="province" class="form-control validform-select Validform_error" onchange="checkprovince(this)">
							...
							@foreach($province as $item)
								<option {{$company->province == $item['id'] ? 'selected': ''}} value="{!! $item['id'] !!}">{!! $item['name'] !!}</option>
							@endforeach
						</select>
					</p>
					...
					<p class="col-sm-4">
						@if(!empty($company->city))
							<select class="form-control  validform-select" name="city" id="city" onchange="checkschoolprovince(this)">
								@else
									<select class="form-control  validform-select" style="display: none;" name="city" id="school_province" onchange="checkschoolprovince(this)">
										@endif
										<option value="" id="school-province-back">请选择城市</option>
										@foreach($province_city as $item4)
											<option {{$company->city == $item4['id'] ? 'selected': ''}} value="{!! $item4['id'] !!}">{!! $item4['name'] !!}</option>
										@endforeach
									</select>
					</p>
							...
							<select class="form-control  validform-select" name="area" id="area">
								@else
									<select class="form-control  validform-select" style="display: none;" name="area" id="area">
										...
										@foreach($city_area as $item5)
											<option {{$company->area == $item5['id'] ? 'selected': ''}} value="{!! $item5['id'] !!}">{!! $item5['name'] !!}</option>
										@endforeach
									</select>
					</p>
				</div>
	...
	<div class="bankAuth-bottom clearfix col-xs-12">
		...
		<p class="col-sm-4">
			<input type="text" placeholder="长度为1-6位" id="form-field-1" name="address"  class="col-xs-10 col-sm-5" value="{{$company->address}}">
			<span class="help-inline col-xs-12 col-sm-7"><i class="light-red ace-icon fa fa-asterisk"></i></span>
		</p>
	</div>
	...
	<div class="bankAuth-bottom clearfix col-xs-12">
		...
		<p class="col-sm-4">
			<input type="text" placeholder="长度为1-6位" id="form-field-1" name="phone"  class="col-xs-10 col-sm-5" value="{{$company->phone}}">
			<span class="help-inline col-xs-12 col-sm-7"><i class="light-red ace-icon fa fa-asterisk"></i></span>
		</p>
	</div>
