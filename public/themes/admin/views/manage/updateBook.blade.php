...
<h3 class="header smaller lighter blue mg-top12 mg-bottom20">更新书籍</h3>

<form class="form-horizontal" action="/manage/updateBook" method="post" enctype="multipart/form-data">
    {{csrf_field()}}
    <div class="g-backrealdetails clearfix bor-border interface">
        ...
        <div class="form-group interface-bottom col-xs-12">
            ...
            <div class="text-left col-sm-8">
                <!--编辑器-->
                <div class="clearfix">
                    <script id="editor" name="book_synopsis"  type="text/plain" style="height:300px;" >{!! htmlspecialchars_decode($book['book_synopsis']) !!}</script>
                </div>
            </div>
        </div>
        <div class="form-group interface-bottom col-xs-12">
            ...
            <div class="text-left col-sm-9">
                <select name="book_type">
                    ...
                    @foreach($book_type as $item)
                        <option value="{{$item['id']}}" @if($book['book_type'] == $item['id']) selected="selected" @endif>{{ $item['name'] }}</option>
                    @endforeach
