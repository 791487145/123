                    ...
                    @foreach($singles as $single)
                        <tr>
                            ...
                            <td rowspan = '{{count($single->result)}}' class="hidden-480">
                                <img src = "/{!! $single->logo !!}" alt = "暂无图片" height="50px" width="50px">
                            </td>
                            <td rowspan = '{{count($single->result)}}'>
                                ...
                                <td>
                                    @if($result->win == 0)
                                        <div class="hidden-sm hidden-xs btn-group">
                                            <a class="btn btn-xs btn-info" href="/manage/game/{{$result->id}}_{{$single->id}}/teamEdit">
...
<script>
    $(".group").click(function () {
        ...
        $.post('/manage/game/hx/sectionalizationDetail',{_token:_token,type:2,competition:competition}, function (msg){
            if(msg == 0){
                alert("您还有比赛未选择结果，不能分组");
                return false;
            }
            alert("分组成功");
            location.reload();
    ...
    $(".binarySectionalization").click(function () {
        ...
        $.post('/manage/game/hx/sectionalization',{_token:_token,type:2,competition:competition,id:munbox}, function (msg){
            alert("分组成功");
            location.reload();
        })
    })
