...
use App\Modules\Manage\Model\InvitingAwardsModel;
...
class PromoteController extends ManageController
{
    ...
    public function promoteConfig()
    {
        ...
        $extend = SystemTasksModel::where('status','valid')
                                  ...
                                  ->get();
        //推广名次
        $amount = SystemTasksModel::where('status','valid')
                                  ->where('type','10')
                                  ->where('reward_type','1')
                                  ->orderBy('amount','desc')
                                  ->select('id','amount')
                                  ...
                                  ->get();
        //推广名次奖励列表
        $awards = InvitingAwardsModel::select('inviting_awards.*','st.amount','u.mobile')
                                     ->where('inviting_awards.status','valid')
                                     ->leftjoin('system_tasks as st','st.id','=','inviting_awards.fid')
                                     ->leftjoin('users as u','u.id','=','inviting_awards.uid')
                                     ->get();
    ...
    //添加推广名次奖励
    public function addInvitingAwards(Request $request)
    {
        $data = $request->except('_token');
        $data['created_at'] = date('Y-m-d H:i:s');
        if($request->get('phone')){
            $uid = UserModel::where('mobile',$data['phone'])->select('id')->first();
            $data['uid'] = $uid['id'];
            unset($data['phone']);
        }
        $info = InvitingAwardsModel::insert($data);
        $return = $info ? '操作成功' : '操作失败';
        return redirect()->back()->with(['error'=>$return]);
    }
    //修改推广名次奖励
    public function invitingAwards(Request $request)
    {
        $data = $request->except('_token');
        foreach($data as $k=>$v){
            if($v['phone']){
                $uid = UserModel::where('mobile',$v['phone'])->select('id')->first();
                $data[$k]['uid'] = $uid['id'];
                unset($data[$k]['phone']);
            }
        }
        $info = InvitingAwardsModel::update($data);
        $return = $info ? '操作成功' : '操作失败';
        return redirect()->back()->with(['error'=>$return]);
    }

    //删除推广名次奖励
    public function deleteInvitingAwards($id)
    {
        $info = InvitingAwardsModel::whereId($id)->update(['status'=>'invalid']);
        $return = $info ? '操作成功' : '操作失败';
        return redirect()->back()->with(['error'=>$return]);
    }
