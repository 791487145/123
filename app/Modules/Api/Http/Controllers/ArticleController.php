...
class ArticleController extends ApiBaseController
{
    ...
    /**推荐中心（get:activeCenter）
     * @param Request $request
     * @return \Illuminate\Http\Response
     */
    public function activeCenter(Request $request)
    {
       /* if(Cache::has('infos_actives_invites_ads'))
        {
            $data = Cache::get('infos_actives_invites_ads');
        }else
        {*/
            //资讯
            $data['infos'] = ArticleModel::where('cat_id', 61)->select('id','created_at','title','summary','view_times','thumb_pic')->orderBy('id', 'desc')->where('display_order', ArticleModel::ARTICLE_ZIXUN)->get();
            if (!$data['infos']->isEmpty()) {
                foreach ($data['infos'] as $v) {
                    $v->created_at = self::timeShow($v->created_at);
                }
                $data['infos'] = $data['infos']->toArray();
            }

            //活动
            $data['actives'] = ArticleModel::where('cat_id', 60)->select('id','title','view_times','thumb_pic')->orderBy('id', 'desc')->where('display_order', ArticleModel::ARTICLE_ZIXUN)->get();
            if (!$data['actives']->isEmpty()) {
                foreach ($data['actives'] as $v) {
                    $v->created_at = self::timeShow($v->created_at);
                }
                $data['actives'] = $data['actives']->toArray();
            }

            //招聘校园
            $data['invites'] = CampusRecruitmentModel::whereStatus('valid')->select('id','post_name','salary','create_at','post_demand','company_name')->orderBy('id', 'desc')->limit(3)->offset(0)->get();
            if (!$data['invites']->isEmpty()) {
                foreach ($data['invites'] as $v) {
                    $v->created_at = self::timeShow($v->created_at);
                }
                $data['invites'] = $data['invites']->toArray();
            }
    ...
    public function articleTypeList(Request $request)
    {
        ...
        if($type == 1){
            $data['invites'] = CampusRecruitmentModel::whereStatus('valid')->select('id','post_name','salary','create_at','post_demand','company_name')->orderBy('id','desc')->limit($limit)->offset($offset)->get();
            if(!$data['invites']->isEmpty()){
                foreach($data['invites'] as $v){
                    $v->created_at = self::timeShow($v->created_at);
                }
        ...
        //资讯
        if($type == 61){
            $data['infos'] = ArticleModel::where('cat_id',$type)->select('id','created_at','title','summary','view_times','thumb_pic')->orderBy('id','desc')->limit($limit)->offset($offset)->get();
            if(!$data['infos']->isEmpty()){
                foreach($data['infos'] as $v){
                    $v->created_at = self::timeShow($v->created_at);
                }
        ...
        //活动
        if($type == 60){
            $data['infos'] = ArticleModel::where('cat_id',$type)->select('id','title','view_times','thumb_pic')->orderBy('id','desc')->limit($limit)->offset($offset)->get();
            if(!$data['infos']->isEmpty()){
                foreach($data['infos'] as $v){
                    $v->created_at = self::timeShow($v->created_at);
                }
