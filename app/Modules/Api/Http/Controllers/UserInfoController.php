...
class UserInfoController extends ApiBaseController
{
    ...
    public function funSort(Request $request)
    {
        ...
        $ret = UserModel::whereId($tokenInfo['uid'])->update(['function_order'=>$sorts]);
        if($ret){
            return $this->formateResponse(1000, '修改成功');
        }

        return $this->formateResponse(100, '修改失败');
    }
     ...
     * num每个人抢到数量
     */
    public function redisAnfqueue(Request $request)
    {
        $uid = mt_rand(1,1000);
        if(Redis::get('div') > 0){
            Redis::decr('div');
            $data['uid'] = $uid;
            $data['num'] = Redis::lpop('num');
            Redis::lpush('amount',$uid.'-'.$data['num']);
            return $this->formateResponse(1000, '抢到金币',$data);
        }

        return $this->formateResponse(1001, '已抢完');

    }

    /**用户基本信息及已发任务(get:/myInfomation/task)
     * @param Request $request
     * @param token
     */
    public function userInfoOfTasks(Request $request)
    {
        ...
        if(count($error0)){
            return $this->formateResponse(1001,$error[0],$error0);
        }

        $i = 0;
        $tokenInfo = Crypt::decrypt(urldecode($request->input('token')));
        $userBalance = UserBalanceModel::where('user_id',$tokenInfo['uid'])->select('id','employer_grade','emp_value','create_task_num')->first();

        $userEmplaysSort = Redis::zrevrange('userEmplaysSort',0,9,'withscores');

        if(!empty($userEmplaysSort)){
            foreach($userEmplaysSort as $k=>$v){
                $i++;
                if($k == $tokenInfo['uid']){
                    $userBalance['userEmplaysSort'] = '第'.$i.'名';
                    break;
                }
            }
        }

        if(!isset($userBalance['userEmplaysSort'])){
            $userBalance['userEmplaysSort'] = '未入榜';
        }

        $myTasks = TaskModel::whereUid($tokenInfo['uid'])->select('id','status','title','created_at')->get();

        if(!$myTasks->isEmpty()){
            foreach($myTasks as $myTask){
                $myTask->status  = TaskModel::getStateCN($myTask->status);
            }
            $myTasks = $myTasks->toArray();
        }

        $data['info'] = $userBalance;
        $data['task'] = $myTasks;
