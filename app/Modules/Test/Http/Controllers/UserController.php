...
class UserController extends ApiBaseController
{
    ...
    public function verifyphone(Request $request)
    {
        ...
        if(count($error)){
            ...
            if($user) return $this->formateResponse(1000,'手机号已经注册',array('is_register'=>1));
            return $this->formateResponse(1000,'手机号尚未注册',array('is_register'=>2));
        }
    }
    ...
    public function register(Request $request)
    {
        ...
        $res =  DB::transaction(function() use ($userArr,$promote_code){
           ...
           // $userBalance->balance_freeze = 0;
            ...
            $uses_promote_code = $userInfo->id + 100000;
        ...
        if(!isset($res)){
            ...
            $url = 'http://pay.zfy0351.cn/index.php/Home/Openim/addUser/uid/'.$res['userInfo']->id;
    ...
    public function login(Request $request){
        ...
        $userInfo = UserModel::where('users.mobile',$request->input('phone'));
        ...
        if($userInfo->status ==  1){
            return $this->formateResponse(1001,'用户还未激活，请激活');
        }
        ...
        if($request->input('password') != $userInfo->password){
        ...
        return $this->formateResponse(1000, '登录成功',array('token'=>$token,'userid'=>'imuser'.$userInfo->id));
     ...
     *
     * */
    public function postPasswordSalt(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'phone' => 'mobile_phone',
        ],[
            'phone.required' => '请输入正确的手机号码格式',
        ]);
        $error = $validator->errors()->all();
        if(count($error)) return $this->formateResponse(1001,$error[0]);
        $userInfo = UserModel::where('mobile',$request->input('phone'))->count();
        if($userInfo){
            $salt = UserModel::where('mobile',$request->input('phone'))->pluck('salt');
            return $this->formateResponse(1000, '成功',array('salt'=>$salt));
        }else{
            return $this->formateResponse(1001, '该账号不存在');
        }
    }
    ...
    public function vertify(Request $request){
        ...
        $userInfo = UserModel::leftjoin('user_detail','users.id','=','user_detail.uid')
                             ...
                             ->select('users.id','users.name','users.salt','users.mobile','user_detail.school')
                             ->first();
        ...
        return $this->formateResponse(1000,'success',array('token' => $token,'salt'=>$userInfo->salt));
    }
    ...
    public function functionImg(Request $request)
    {
        $data = [
            'menu' => [
                ...
                ['name' => '江湖', 'img'=> 'uploads/icon/menu/tabbar_icon_24.png', 'selected' => 'uploads/icon/menu/tabbar_icon_46.png', 'sort'=> 2],
                ['name' => '发现', 'img'=> 'uploads/icon/menu/tabbar_icon_26.png', 'selected' => 'uploads/icon/menu/tabbar_icon_47.png', 'sort'=> 3],
                ['name' => '个人', 'img'=> 'uploads/icon/menu/tabbar_icon_28.png', 'selected' => 'uploads/icon/menu/tabbar_icon_48.png', 'sort'=> 4],
            ],
            'background' => '#ffffff',
            'line' => '#ececec',
            'word'  =>  '#929292',
     ...
     *  安卓第三方登录
     * */
    public function postOauthLogin(Request $request){
        $data = $request->all();
        if(empty($data['type'])){
            return $this->formateResponse(1001,'请选择登录方式');
        }
        if($data['type'] == 'qq'){
            $user = UserModel::where('qq_openid',$data['qq_openid'])->count();
            if($user){
                $userInfo = UserModel::where('users.qq_openid',$data['qq_openid'])
                    ->leftjoin('user_detail','users.id','=','user_detail.uid')
                    ->select('users.id','users.name','users.mobile','user_detail.school')
                    ->first();

            }else{
                $info = self::addData($data);
                if($info['msg'] == 'success' ){
                    $token = self::createToken($info['id'],'',$data['username'],'');
                    $url = 'http://pay.zfy0351.cn/index.php/Home/Openim/addUser/uid/'.$info['id'];
                    file_get_contents($url);
                    return $this->formateResponse(1000, '登录成功',array('token'=>$token,'userid' => 'imuser'.$info['id']));
                }else{
                    return $this->formateResponse(1001, '登录失败');
                }
            }
        }
        if($data['type'] == 'weixin'){
            $user = UserModel::where('wx_openid',$data['wx_openid'])->count();
            if($user){
                $userInfo = UserModel::where('users.wx_openid',$data['wx_openid'])
                    ->leftjoin('user_detail','users.id','=','user_detail.uid')
                    ->select('users.id','users.name','users.mobile','user_detail.school')
    ...
    public function addData($info){
        ...
        $res = DB::transaction(function () use ($data) {
            ...
            $id = $userInfo->id;
            $detail['uid'] = $id;
            UserDetailModel::insert($detail);
            $balance['user_id'] = $id;
            UserBalanceModel::insert($balance);
            return $id;
        });
        if ($res) {
            $data['msg'] = 'success';
