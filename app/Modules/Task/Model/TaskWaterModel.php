...
class TaskWaterModel extends Model
{
    ...
    protected $fillable = [
        ...
        'hunter_grade_limit','identify','user_grade','task_cate_name','avatar','schoolName'
    ];
    ...
    {
        $cate = TaskCateModel::orderByRaw('RAND()')->take(1)->first();
        $school = DistrictRegionModel::where('type',0)->orderByRaw('RAND()')->take(1)->first();
        $schoolName = DistrictRegionModel::getAllfromSchoolId($school->id);
        $begintime = date('Y-m-d',strtotime('-1 day'));

        $key = array_rand(Lang::get("auth.title"));
        $title = Lang::get('auth.title.'.$key.'.title.'.array_rand(Lang::get('auth.title.'.$key.'.title')));
        $desc = Lang::get('auth.title.'.$key.'.desc.'.array_rand(Lang::get('auth.title.'.$key.'.desc')));
        $price = Lang::get('auth.title.'.$key.'.price.'.array_rand(Lang::get('auth.title.'.$key.'.price')));

       if($key == 'supermarket'|| $key == 'food'||$key == 'review'||$key == 'used'){
           $desc = str_replace('xxx',$title,$desc);
       }

        $task  = new TaskWaterModel();
        $task->title = $title;
        $task->desc = $desc;
        $task->cate_id = $cate->id;
        $task->type_id = 1;
        $task->phone = 110;
        $task->schoolName = DistrictRegionModel::findByIdGetName($school->id);
        $task->region_limit = Lang::get('auth.region_limit.'.array_rand(Lang::get("auth.region_limit")));

        if($task->region_limit != 0){
            $task->region_limit = $schoolName;
        }else{
            $task->region_limit = '不限';
        }

        $task->status = 2;
        $task->bounty = $price;
        $task->bounty_status = 1;
        $task->publicity_at = date("Y-m-d H:i:s",(strtotime("now") + Lang::get('auth.publicity_at.'.array_rand(Lang::get("auth.publicity_at")))*3600));
        $task->deposit_cash = $task->bounty;
        $task->view_count = 0;
        $task->uid = 1;
        $task->username = Lang::get('auth.falseName.'.array_rand(Lang::get("auth.falseName")));
        $task->worker_num = 0;
        $task->hunter_grade_limit = 0;
        ...
        $task->user_grade = Lang::get('auth.user_grade.'.array_rand(Lang::get("auth.user_grade")));
        $task->task_cate_name = $cate->name;
        $task->avatar = 'uploads/head_img/'. mt_rand(1,63).'.jpg';
        $task->identify = 9;
        $task->created_at = ApiBaseController::randomDate($begintime);
        $task->save();
     ...
     * @return mixed
     */
    static public function createTaskOne($data)
    {
        $cate = TaskCateModel::orderByRaw('RAND()')->take(1)->first();
        $begintime = date('Y-m-d',strtotime('-1 day'));
        ...
        //$task->schoolName = DistrictRegionModel::findByIdGetName($school->id);
        $task->region_limit = $data['region_limit_num'];

        if($task->region_limit != 0){
            $task->schoolName = DistrictRegionModel::findByIdGetName($data['region_limit']);
            $task->region_limit = $task->schoolName;
        }else{
            $task->region_limit = '不限';
            $school = DistrictRegionModel::where('type',0)->orderByRaw('RAND()')->take(1)->first();
            $schoolName = DistrictRegionModel::getAllfromSchoolId($school->id);
            $task->schoolName = $schoolName;
        }

        $task->status = 2;
        $task->bounty = $data['bounty'];
        $task->bounty_status = 1;
        $task->publicity_at = date("Y-m-d H:i:s",strtotime(date("Y-m-d H:i:s")) + $data['publicity_at']*3600);
        $task->deposit_cash = $task->bounty;
        $task->view_count = 0;
        $task->uid = 1;
        ...
        $task->task_success_draw_ratio = 0;
        //$task->created_at = date("Y-m-d H:i:s");
        $task->user_grade = $data['user_grade'];
        $task->task_cate_name = $cate->name;
        $task->avatar = $data['image'];
        $task->identify = 9;
        $task->created_at = ApiBaseController::randomDate($begintime);
        $task->save();
    ...
    {
        $taskDetail = self::whereId($id)->select('id','uid','desc','work_time','title','created_at','cate_id','phone','bounty','status','area','region','province')->first()->toArray();
        ...
        $taskDetail['work_time'] = ($taskDetail['work_time'] == '0') ? '不限' : $taskDetail['work_time'];
